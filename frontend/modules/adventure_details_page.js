...
function addAdventureDetailsToDOM(adventure) {
  ...
  adventure.images.map((img) => {
    ...
    // console.log(img)
    photoDiv.className = "col-12"
    photoDiv.setAttribute = ('id', 'photoGallery')
    photoDiv.innerHTML = `
      <img src="${img}" class = "activity-card-image img-responsive"/>
    `;
...
function addBootstrapPhotoGallery(images) {
  ...
  images.map((imgSrc, index) => {
    let photoInsert = document.createElement('div')
    photoInsert.className = `carousel-item ${index === 0 ? "active" : ""}`
    photoInsert.innerHTML = `<img src=${imgSrc} alt="" srcset="" class="activity-card-image pb-3 pb-md-0"/>`;

    // console.log(photoInsert)
    // newDivELe.append(photoInsert)
    document.getElementById('carousel-inner').appendChild(photoInsert);
  })
}
